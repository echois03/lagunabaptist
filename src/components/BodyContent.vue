<template>
<div id="content">
  <div class="justify-content-center">
    <b-row id="section-callout" class="text-start mx-0">
      <b-col class="my-md-5 py-4 align-self-center col-sm-12 col-md-6 v-border-right">
        <h2>Church Family Conference 2022</h2>
      </b-col>
      <b-col class="p-5 col-sm-12 col-md-6">
        <p>The CFC is an annual church-wide event hosted at our church with the aim of blessing LCBC with biblical teaching on a specific, relevant topic. We are very thankful for how well last year's CFC turned out, and we hope and pray this can continue to become a yearly tradition for us. This year, the topic we want to address is what the Bible has to say on mental illness. To that end, we have invited Dr. Charles Hodges to speak.</p>
      </b-col>
    </b-row>
    <b-row id="speaker" class="my-4 py-5">
      <b-col lg="4" sm="5" class="pe-4 pb-5"><img src="https://www.csmedia1.com/lagunabaptist.org/charles-hodges-pf.jpg">
      </b-col>
      <b-col lg="8" sm="7" class="pb-0 pr-lg-5 ps-4 text-start">
        <h2 id="overview">Dr. Charles Hodges</h2>
        <p>Charles Hodges is a family practice physician as well as a biblical counselor, instructor, and conference speaker. He practices medicine in Indianapolis, IN for Ascension St. Vincent Health and is board certified in Family Medicine and Geriatrics. He is also a licensed marital family therapist. Dr. Hodges serves as the Executive Director of Vision of Hope, which is a residential counseling facility in Lafayette, IN for women who struggle with eating disorders, self-harm, OCD, substance abuse, and other problems. He also teaches and counsels at the Faith Biblical Counseling Ministry and teaches on medical subjects in biblical counseling around the country and abroad.</p>
        <p>Dr. Hodges is the author of <em>Good Mood, Bad Mood: Help and Hope for Depression and Bipolar Disorder</em>, and he has a blog at <a href="http://www.goodmoodbadmood.com/" target="_blank" rel="noopener noreferrer">goodmoodbadmood.com</a> where he writes about medicine and biblical counseling. He has also contributed to various biblical counseling blogs, including the <a href="https://www.biblicalcounselingcoalition.org/" target="_blank" rel="noopener noreferrer">Biblical Counseling Coalition</a> and the <a href="https://ibcd.org/" target="_blank" rel="noopener noreferrer">Institute for Biblical Counseling &amp; Discipleship</a>. He and his wife, Helen, have been married 50 years and have 4 children and 13 grandchildren. In his free time, he enjoys golf and running, as well as making frequent trips to Disney with his family.</p>
        <h6>Credentials:</h6>
        <ul>
          <li>B.A. – History, Indiana University-Purdue University of Indianapolis</li>
          <li>M.D. – Indiana University School of Medicine</li>
          <li>M.A. – Counseling, Liberty University</li>
          <li>M.A. – Religion, Liberty Baptist Theological Seminary</li>
        </ul>
      </b-col>
    </b-row>
    <h2 id="schedule" class="pb-5 text-start">Schedule</h2>
    <b-row class="text-start">
      <b-col cols="12" sm="4" class="pl-0 pb-sm-0 pb-3 pr-5 v-border-right">
        <b-row class="day pb-3">
          <p><strong>Friday, September 9</strong></p>
        </b-row>
        <b-row class="pb-0">
          <b-col cols="5" class="pl-0">
            <p>Session 1:</p>
          </b-col>
          <b-col class="px-0">
            <p>7:00 - 8:30PM</p>
          </b-col>
        </b-row>
      </b-col>
      <b-col cols="12" sm="4" class="pb-sm-0 pb-3 px-sm-5 v-border-right">
        <b-row class="day pb-3">
          <p><strong>Saturday, September 10</strong></p>
        </b-row>
        <b-row class="pb-0">
          <b-col lg="5" md="6" cols="5" class="pl-0">
            <p>Sessions 2&3:</p>
            <p>Lunch Break:</p>
            <p>Session 4:</p>
          </b-col>
          <b-col class="px-0">
            <p>9:00 - 11:35AM</p>
            <p>11:35AM - 1:00PM</p>
            <p>1:00 - 2:30PM</p>
          </b-col>
        </b-row>
      </b-col>
      <b-col class="pb-0 ps-sm-5">
        <b-row class="day pb-3">
          <p><strong>Sunday, September 11</strong></p>
        </b-row>
        <b-row align-h="start" class="pb-0">
            <p>Join us for Sunday worship at <em>9am</em>!</p>
            <p>A Q&A session for the conference will take place at <em>10:45am</em> during our church's Sunday school hour. The session will be recorded for those who are not able to attend.</p>
        </b-row>
      </b-col>
    </b-row>
    <b-row class="py-5 text-start">
      <h2 id="faqs">FAQs</h2>
      <div class="accordion pb-0" role="tablist">
        <b-card no-body>
          <b-card-header header-tag="header" class="p-1" role="tab">
            <b-button block v-b-toggle.accordion-1 variant="light" class="FAQs"> +      Is there a cost for attending?</b-button>
          </b-card-header>
          <b-collapse id="accordion-1" accordion="my-accordion" role="tabpanel">
            <b-card-body>
              <b-card-text>There will be a small fee of $20 per adult who attends ($25 per adult for non-members). This will help cover costs for refreshments, lunch, and books that we will be giving-away to all attendees.
              </b-card-text>
            </b-card-body>
          </b-collapse>
        </b-card>
      </div>
      <div class="accordion pb-0" role="tablist">
          <b-card no-body>
            <b-card-header header-tag="header" class="p-1" role="tab">
              <b-button block v-b-toggle.accordion-2 variant="light" class="FAQs"> +      Where should I go when I arrive?</b-button>
            </b-card-header>
            <b-collapse id="accordion-2" accordion="my-accordion" role="tabpanel">
              <b-card-body>
                <b-card-text>Upon arrival, check-in at the registration table in the MPR (the white building) where main events will be hosted. Childcare will be located in the ETC (the red building).</b-card-text>
              </b-card-body>
            </b-collapse>
          </b-card>
        </div>
      <div class="accordion pb-0" role="tablist">
          <b-card no-body>
            <b-card-header header-tag="header" class="p-1" role="tab">
              <b-button block v-b-toggle.accordion-3 variant="light" class="FAQs"> +      Is there childcare?</b-button>
            </b-card-header>
            <b-collapse id="accordion-3" accordion="my-accordion" role="tabpanel">
              <b-card-body>
                <b-card-text>On-site childcare is available for kids 3rd grade and under. We will be providing snacks for childcare, but we ask parents to bring snacks for their children if they have any allergy concerns. We will be observing recommended CDC guidelines such as wearing masks indoors and frequent hand-washing. Kids 4th grade and above are welcome to attend sessions with their parents.</b-card-text>
              </b-card-body>
            </b-collapse>
          </b-card>
        </div>
      <div class="accordion pb-0" role="tablist">
        <b-card no-body>
          <b-card-header header-tag="header" class="p-1" role="tab">
            <b-button block v-b-toggle.accordion-4 variant="light" class="FAQs"> +      What is included in the registration?</b-button>
          </b-card-header>
          <b-collapse id="accordion-4" accordion="my-accordion" role="tabpanel">
            <b-card-body>
              <b-card-text>Your registration cost of $10 per adult will include the following:
              <ul>
                <li>Saturday morning snacks and refreshments, as well as Saturday lunch</li>
                <li>Resources on the topic of a biblical response to mental illness that we recommend</li>
              </ul></b-card-text>
            </b-card-body>
          </b-collapse>
        </b-card>
      </div>
      <div class="accordion pb-0" role="tablist">
        <b-card no-body>
          <b-card-header header-tag="header" class="p-1" role="tab">
            <b-button block v-b-toggle.accordion-5 variant="light" class="FAQs"> +      When do I pay my registration fee?</b-button>
          </b-card-header>
          <b-collapse id="accordion-5" accordion="my-accordion" role="tabpanel">
            <b-card-body>
              <b-card-text>There are two ways you can pay: pay online when you register to attend, or pay at the door when you arrive in-person for CFC. If you choose to pay at the door, select "Pay Later" when you are registering online.</b-card-text>
            </b-card-body>
          </b-collapse>
        </b-card>
      </div>
      <div class="accordion pb-0" role="tablist">
        <b-card no-body>
          <b-card-header header-tag="header" class="p-1" role="tab">
            <b-button block v-b-toggle.accordion-6 variant="light" class="FAQs"> +      When is the last day I can register?</b-button>
          </b-card-header>
          <b-collapse id="accordion-6" accordion="my-accordion" role="tabpanel">
            <b-card-body>
              <b-card-text>We will keep registration open until the day of the CFC, but if we get enough registrations where we would max out the room capacity, then we would need to close registration at that point.</b-card-text>
            </b-card-body>
          </b-collapse>
        </b-card>
      </div>
      <div class="accordion pb-0" role="tablist">
          <b-card no-body>
            <b-card-header header-tag="header"  class="p-1" role="tab">
              <b-button block v-b-toggle.accordion-7 variant="light" class="FAQs"> +      Will there be a live-stream to watch online?</b-button>
            </b-card-header>
            <b-collapse id="accordion-7" accordion="my-accordion" role="tabpanel">
              <b-card-body>
                <b-card-text>We do not plan to live-stream the conference. We encourage everyone to come out in-person for the full-experience. However, we do plan to record the sessions and make them available at a later date.</b-card-text>
              </b-card-body>
            </b-collapse>
          </b-card>
        </div>  
      <div class="accordion pb-0" role="tablist">
        <b-card no-body>
          <b-card-header header-tag="header"  class="p-1" role="tab">
            <b-button block v-b-toggle.accordion-8 variant="light" class="FAQs">+ Can I invite other people who do not attend LCBC?</b-button>
          </b-card-header>
          <b-collapse id="accordion-8" accordion="my-accordion" role="tabpanel">
            <b-card-body>
              <b-card-text>The CFC is intended primarily for our church family at LCBC. However, we hope we can bless others from other church families as well. To that end, the registration timeframe will be as follows:
              <ul>
                <li>July 10 - Aug. 9: Open registration for all LCBC members and attendees</li>
                <li>Aug. 10 - Sept. 9: Open registration for anyone</li>
              </ul></b-card-text>
            </b-card-body>
          </b-collapse>
        </b-card>
      </div>
    </b-row>
    <b-button class="btn register primary mt-3" variant="primary" href="https://lagunabaptist.churchcenter.com/registrations/events/1351974">Register Now</b-button>
  </div>
</div>
</template>